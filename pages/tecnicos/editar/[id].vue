    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6">
      ...
      <div class="mb-8">
        <div class="flex items-center gap-2 text-sm text-gray-500 mb-4">
          ...
          <span class="text-gray-800 font-medium">Editar técnico</span>
        </div>
        
        <div class="flex justify-between items-end">
          <div>
            <h1 class="text-2xl font-semibold text-gray-900">Editar Técnico</h1>
            <p class="text-gray-500 text-sm mt-1">Actualice la información del técnico</p>
          </div>
        </div>
      </div>
      ...
      <form v-else @submit.prevent="handleActualizar" class="space-y-6">
        <!-- Tarjeta principal -->
          ...
          <div class="px-6 py-4 bg-gradient-to-r from-blue-50 to-white border-b border-gray-100">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-xs text-gray-500">Datos del técnico especialista</p>
              </div>
            </div>
          </div>
        ...
        <div class="bg-blue-50 rounded-lg border border-blue-100 p-4">
          <div class="flex items-start gap-3">
            ...
            <div class="text-sm text-blue-800">
              ...
              <p class="text-xs text-blue-700">Los cambios se reflejarán en todas las reparaciones asignadas a este técnico.</p>
            </div>
          </div>
        </div>
        ...
        <div class="flex justify-end gap-3 pt-4">
          ...
          <button
            type="submit"
            :disabled="guardando"
            class="px-6 py-2.5 text-sm font-medium text-white bg-blue-600 rounded-lg hover:bg-blue-700 transition disabled:opacity-50 flex items-center gap-2 shadow-sm"
          >
            <i v-if="guardando" class="ri-loader-4-line animate-spin"></i>
            <i v-else class="ri-save-line"></i>
            {{ guardando ? 'Guardando...' : 'Actualizar Técnico' }}
          </button>
        </div>
      </form>
    </div>
...
const { actualizarTecnico } = useTecnicos()

const cargando = ref(true)
const guardando = ref(false)
const tecnicoId = route.params.id
...
onMounted(async () => {
  cargando.value = true
  try {
    const tecnicos = await $fetch('/api/tecnicos')
    const tecnico = tecnicos.find(t => t.id === tecnicoId)
    
    if (tecnico) {
      form.nombre = tecnico.nombre
      form.especialidad = tecnico.especialidad || ''
      form.telefono = tecnico.telefono || ''
    } else {
      mostrarToast('Técnico no encontrado', 'error')
      setTimeout(() => {
...
const handleActualizar = async () => {
  ...
  try {
    const result = await actualizarTecnico(tecnicoId, form)
    
    if (result.success) {
      mostrarToast('Técnico actualizado correctamente', 'success')
      setTimeout(() => {
        router.push('/tecnicos')
      }, 1500)
    } else {
      mostrarToast(result.error || 'Error al actualizar el técnico', 'error')
    }
  } catch (error) {
    console.error('Error:', error)
    mostrarToast('Error al actualizar el técnico', 'error')
  } finally {
