    ...
    <div class="max-w-4xl mx-auto py-8 px-4 sm:px-6">
      ...
      <div class="mb-8">
        <div class="flex items-center gap-2 text-[10px] font-mono text-gray-400 uppercase tracking-widest mb-4">
          ...
          <span class="text-[#065F46] font-black">EDITAR_REGISTRO</span>
        </div>
        
        <div class="flex items-center gap-3">
          <div class="h-10 w-1.5 bg-[#10B981] rounded-full"></div>
          <div>
            <h1 class="text-3xl font-black text-[#065F46] tracking-tighter uppercase">Editar Técnico</h1>
            <p class="text-[10px] font-mono text-gray-400 uppercase tracking-widest">Protocolo: TECH_EDIT_001</p>
          </div>
        </div>
      </div>
      ...
      <div v-if="cargando" class="flex flex-col items-center justify-center py-20 bg-white rounded-xl border border-[#D1D5DB]">
        ...
        <span class="text-xs font-mono text-gray-400 uppercase">Loading_Data...</span>
      </div>
      ...
      <form v-else @submit.prevent="handleActualizar" class="space-y-6">
          ...
          <div class="px-6 py-4 bg-[#F8FAFC] border-b border-[#D1D5DB]">
            <div class="flex items-center gap-3">
              <div class="w-10 h-10 rounded-lg bg-[#ECFDF5] flex items-center justify-center">
                <i class="ri-user-settings-line text-[#10B981] text-xl"></i>
              </div>
              <div>
                <h2 class="text-[10px] font-black text-[#065F46] uppercase tracking-widest">Ficha_Técnica_Operador</h2>
                <p class="text-[9px] font-mono text-gray-400">Actualización de datos del técnico</p>
              </div>
            </div>
          </div>
          ...
          <div class="flex items-start gap-3">
            ...
            <div>
              ...
              <p class="text-[11px] text-white/80 leading-relaxed">
                Los cambios se reflejarán en todas las reparaciones asignadas a este técnico.
              </p>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col sm:flex-row justify-end gap-4 pt-6">
          ...
          >
            ...
            {{ guardando ? 'PROCESANDO...' : 'ACTUALIZAR_REGISTRO' }}
          </button>
          <NuxtLink
            to="/tecnicos"
...
onMounted(async () => {
  ...
  try {
    ...
    if (tecnico) {
      ...
      mostrarToast('REGISTRO_NO_ENCONTRADO', 'error')
      setTimeout(() => {
...
const handleActualizar = async () => {
  ...
  try {
    ...
    if (result.success) {
      mostrarToast('REGISTRO_ACTUALIZADO_EXITOSAMENTE', 'success')
      setTimeout(() => {
        router.push('/tecnicos')
      }, 1500)
    } else {
    ...
    mostrarToast('ERROR_CRÍTICO_AL_ACTUALIZAR', 'error')
  } finally {
